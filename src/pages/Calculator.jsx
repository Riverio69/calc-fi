...
export default function Calculator() {
  ...
  const handleGetQuote = () => {
    ...
    const message = [
      `Hei! Haluaisin pyytää tarjouksen projektista.`,
      ``,
      `Projektityyppi: ${projectType?.label || 'Ei valittu'}`,
      `Lisäosat: ${features.length > 0 ? features.map(f => f.label).join(', ') : 'Ei mitään'}`,
      `Laatu: ${quality.label} (×${quality.multiplier})`,
